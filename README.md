...
signed with is always in the set the server publishes. Bearer tokens verify against the
published keys parsed once per node under that version, and the user row an
authenticated request reads is kept per node for five seconds (never in Valkey: it
carries the password hash), evicted on every node by any write to that user. The
in-process cache holds at most 100,000 entries (least used go first) and loads a missing
key once however many requests ask for it at the same time. RSA JWT and SAML signatures
run on the blocking pool. Every hot query runs on an index
...
| `ridm_audit_events_total`, `ridm_audit_events_dropped_total`, `ridm_audit_parked_events`, `ridm_audit_queue_depth`, `ridm_webhook_dispatch_queue_depth`, `ridm_audit_sink_rows_total`, `ridm_audit_sink_failures_total`, `ridm_audit_sink_lag_rows` | | the audit writer and its export sink |
| `ridm_event_queue_depth{subscriber}`, `ridm_event_queue_capacity{subscriber}`, `ridm_event_queue_dropped_total{subscriber}` | | the bounded durable event queues (`/readyz` reports `events: saturated` past 80%) |
...
Webhook deliveries are queued by an in-process subscriber of the event bus (a durable
one: a burst delays events, never drops them; `ridm_webhook_dispatch_queue_depth`) and
sent at once (one prompt pass per tenant at a time, under a short Valkey lock; on every node,
...
in-process writer with its own queue (nothing is skipped under load; it appends what has
queued up in one transaction per chain; `ridm_audit_queue_depth`); rows are hash-chained per tenant (`SHA-256(prev_hash || row)`), so a
