...
| `UI_URL` | URL | `PUBLIC_URL` | Base URL of the sign-in, consent and console pages. The server redirects browsers to pages under it (`/login/`, `/consent/`, `/console/`, ...) and derives the built-in console clients' redirect URIs from it, updating them at start-up when it changes. Leave it unset when the server serves its embedded UI; set it to wherever the UI is hosted otherwise. |
| `EMBEDDED_UI` | boolean | `true` | Serve the UI compiled into the binary (the container image's build, `--features embedded-ui`) on `PUBLIC_URL`'s origin. Has no effect on a binary built without the feature, or when `UI_URL` is another origin, and `false` turns the pages off on a node that should answer the API alone. See [Deployment overview](../deploy/overview.md#where-the-ui-is-served-from). |
...
| `SECURITY_CONTACT` | URI list | unset | Comma-separated `mailto:`, `tel:` or `https:` URIs, written as the `Contact` fields of `/.well-known/security.txt` ([RFC 9116](https://www.rfc-editor.org/rfc/rfc9116)), with an `Expires` 30 days ahead, rolling. Unset (with `SECURITY_TXT` unset too), that path answers `404`: the document names whoever handles vulnerability reports for *this* deployment, which only its operator can say. |
| `SECURITY_POLICY_URL` | https URL | unset | Written as the document's `Policy` field. Needs `SECURITY_CONTACT`. |
| `SECURITY_TXT` or `SECURITY_TXT_FILE` | text | unset | The whole document, served byte for byte, for an operator who writes (or OpenPGP-signs) their own. It must hold at least one `Contact` and exactly one RFC 3339 `Expires`; the server warns at start-up once that date has passed, and keeps serving it. Cannot be combined with the two above. |
