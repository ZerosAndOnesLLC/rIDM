...
| `broker` | identity brokering with rIDM as its own upstream provider: an administrator adds it in the console (preset-free, discovered from the issuer), the login page offers "Continue with …" and signs the user in through it (linking by verified email), the account console unlinks and links it again, the user detail lists the identity, the provider is deleted |
| `pats` | personal access tokens: minted in the account console (name, scopes, expiry; shown once), used as a bearer on the account and admin APIs within its scopes, revoked from the console and refused afterwards |
...
| `console-a11y` | axe pass over every console page in its landing state (all pages light, the first eight dark, six at phone width with no horizontal overflow) |
...
| `console-saml` | SAML identity provider: IdP URLs and signing keys shown, a service provider registered by hand, its NameID format, an attribute and IdP-initiated sign-in saved as edited, the launcher link signing in and posting a signed response to the SP (its consumer URL stood in for by a route that captures the post), deletion |
