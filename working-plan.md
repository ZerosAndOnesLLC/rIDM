...
- [ ] 0.2 `config.rs` (typed env: DATABASE_URL, REDIS_URL, PUBLIC_URL, MASTER_KEY or MASTER_KEY_FILE, BIND_ADDR, LOG_FORMAT, DOCS_ENABLED, COOKIE_SECURE, TRUSTED_PROXIES, TLS_CERT/TLS_KEY), `error.rs`, health endpoints, tracing setup.
...
- [ ] 2.5 Refresh token service: opaque, hashed, rotation, family reuse detection; `jti` denylist for instant access-token revocation.
...
- [ ] 3.2 Discovery document, complete and accurate for every supported feature.
