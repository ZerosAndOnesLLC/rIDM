...
| A captured Kerberos Negotiate header replayed to sign in again | Accepted authenticators are remembered in Valkey (per tenant, for twice the allowed skew) and refused a second time; the authenticator's time must be within the skew. Regression test `tests/security/kerberos.rs` |
| A forged or altered Kerberos ticket, or one for another service or tenant | The ticket must decrypt (AES with its HMAC checked) under the provider's own keytab key for the service principal it names; the authenticator must decrypt under the ticket's session key and name the ticket's client; RC4, DES and user-to-user tickets are refused. Regression tests `tests/security/kerberos.rs`, a flip-every-byte test and the `kerberos_token` fuzz target |
...
| A download ticket (an export URL that authenticates without the header) leaked or reused | Issued only to an authenticated administrator who holds what the export requires; 256 random bits stored only by hash; one `GET` of exactly the path and query it names, within 60 s, spent on first presentation anywhere; redemption re-runs the whole admin authentication with the caller's token (live session, active user, current permissions). Only the export routes accept one. Regression tests `tests/security/download_tickets.rs` |
