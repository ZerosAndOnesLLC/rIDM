...
| Login CSRF through the SAML assertion consumer service | Responses are accepted only for a request rIDM sent (unsolicited ones are a per-provider opt-in), and the same-site continue step requires the binding cookie set in the browser that started the sign-in |
...
- **SAML logout is front-channel only.** SPs are logged out through the user's browser;
  ...
  and a sign-out started by a downstream SAML SP does not reach the upstream IdP.
