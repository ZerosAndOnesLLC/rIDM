-- SCIM 2.0 provisioning (Phase 9.6).

-- The provisioning system's own identifier for a user (SCIM externalId).
ALTER TABLE users ADD COLUMN external_id text;
CREATE UNIQUE INDEX users_external_id_key ON users (tenant_id, external_id)
    WHERE external_id IS NOT NULL AND deleted_at IS NULL;

-- Bearer tokens a provisioning system uses against /scim/v2/{tenant}.
CREATE TABLE scim_tokens (
    ...
    CONSTRAINT scim_tokens_name_length CHECK (length(name) BETWEEN 1 AND 100)
);
CREATE INDEX scim_tokens_tenant_idx ON scim_tokens (tenant_id, created_at DESC);
SELECT enable_tenant_rls('scim_tokens');

-- Permission catalogue: ridm:scim:read / ridm:scim:write (user managers hold both).
