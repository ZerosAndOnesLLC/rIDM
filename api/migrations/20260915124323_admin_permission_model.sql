-- 0014: Admin permission model.
--
-- Every tenant gets a built-in resource server `urn:ridm:admin` whose
-- permissions (`ridm:<resource>:<action>`) guard the admin API, plus the five
-- built-in admin roles. Global administrators hold these roles in `master`;
-- per-tenant administrators hold them in their own tenant. The catalogue is
-- mirrored in `services/admin_access.rs`; a contract test keeps the two equal.
--
-- Built-in rows are flagged so the services refuse to rename or delete them.

SELECT set_config('app.bypass_rls', 'on', true);

ALTER TABLE roles ADD COLUMN built_in boolean NOT NULL DEFAULT false;
ALTER TABLE resource_servers ADD COLUMN built_in boolean NOT NULL DEFAULT false;

-- Idempotent: safe to call again after the catalogue grows in a later
-- migration (new permissions are added, existing rows untouched, built-in
-- roles receive the new grants).
     ...
     WHERE r.tenant_id = p_tenant_id AND r.client_id IS NULL AND r.built_in AND (
        ...
        OR (r.name = 'ridm:user-manager' AND p.name IN
              ('ridm:tenants:read', 'ridm:users:read', 'ridm:users:write',
               ...
               'ridm:groups:read', 'ridm:groups:write', 'ridm:roles:read', 'ridm:audit:read'))
...
CREATE TRIGGER tenants_seed_admin_model AFTER INSERT ON tenants
    FOR EACH ROW EXECUTE FUNCTION tenants_seed_admin_model_trigger();
